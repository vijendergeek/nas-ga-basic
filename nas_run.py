...
train_subset = Subset(trainset, range(device.type == 'cpu' and 1000 or 5000))
val_subset = Subset(valset, range(device.type == 'cpu' and 1000 or 5000))

train_loader = DataLoader(train_subset, batch_size=(device.type == 'cpu' and 32 or 256), shuffle=True)
val_loader = DataLoader(val_subset, batch_size=(device.type == 'cpu' and 32 or 256), shuffle=False)

# Run NAS with GA
ga = GeneticAlgorithm(
    population_size=(device.type == 'cpu' and 5 or 10),  # Small population for demonstration
    generations=(device.type == 'cpu' and 3 or 5),       # Few generations for quick results
    mutation_rate=0.3,
    crossover_rate=0.7
)
